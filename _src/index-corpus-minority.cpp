...
int main() {
	...
	for (; std::getline(std::cin, line); ++cnt) {
		...
		if (line[0] == '<' && line[1] == 's' && line[2] == ' ') {
			...
			last[Y] = 2;
			if (line.find(" minority=\"1\"") != std::string::npos) {
				last[Y] = 1;
			}
		...
		if (rx_word.find()) {
			++total[Words];
			++total_ys[last[Y]][Words];
			tally = true;
		}
		else if (rx_number.find()) {
			++total[Numbers];
			++total_ys[last[Y]][Numbers];
			tally = true;
		}
		else if (rx_alnum.find()) {
			++total[Alnums];
			++total_ys[last[Y]][Alnums];
			tally = true;
		}
		else if (rx_punct.find()) {
			++total[Puncts];
			++total_ys[last[Y]][Puncts];
		}
		else {
			++total[Others];
			++total_ys[last[Y]][Others];
		}
		...
		if (tally) {
			...
			++freq_ys[last[Y]][word][m[0]];
			++freq_ys[last[Y]][lex][m[1]];
			++freq_ys[last[Y]][word_lc][lc[0]];
			++freq_ys[last[Y]][lex_lc][lc[1]];
			++freq_ys[last[Y]][word_nd][nd[0]];
			++freq_ys[last[Y]][lex_nd][nd[1]];
		}
	}
...
CREATE TABLE counts (
	...
	for (auto& ts : total_ys) {
		fprintf(out, "%zu", ts.first);
	...
	for (auto& fts : freq_ys) {
		for (size_t i=0 ; i<N_Fields ; ++i) {
			auto tname = concat("freq_", std::to_string(fts.first), "_", fNames[i]);
			sql.append(concat("CREATE TABLE ", tname, R"( (
	f_text TEXT NOT NULL,
	f_abs INTEGER NOT NULL,
	f_rel REAL NOT NULL,
